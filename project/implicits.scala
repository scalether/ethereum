...
object implicits {
  ...
  implicit class RichProject(project: Project) {
    ...
    def publish: Project = project.settings(
      credentials += Credentials("", "nexus-ext.ethereum.int", sys.env.get("USERNAME").getOrElse(""), sys.env.get("PASSWORD").getOrElse("")),
      publishTo := Some(("releases": RepositoryName) at "http://nexus-ext.ethereum.int/repository/maven-releases/" withAllowInsecureProtocol true)
    )

    def common: Project = publish.settings(
      organization := "org.ethereum.rpc",
    ...
    def scalether: Project = common.settings(
      organization := "org.ethereum.scalether"
    )

    def bitcoin: Project = common.settings(
      organization := "org.ethereum.bitcoin.rpc"
    )

    def blockchain: Project = common.settings(
      organization := "org.ethereum.blockchain"
    )
  }
}
